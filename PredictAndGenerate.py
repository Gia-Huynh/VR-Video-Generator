...
# Initialize logging
def inference_worker (in_queue_list, out_queue_list, notify_queue_list, DEVICE, args_god):
    def inference_worker_nested (in_queue_list, out_queue_list, notify_queue_list, DEVICE):
        print_flush (args_god.encoder, args_god.encoder_path, DEVICE)
        #vits, vitb, vitl each output different depth img scale
        if args_god.encoder == 'vits': #depth.max() around 8-9
            scaler = 1.618 #I'm feeling like it ya know, tbh should have been 1.66
        elif args_god.encoder == 'vitb': #depth.max() around 16-18
            scaler = 0.8 #Feeling like it
        elif args_god.encoder == 'vitl':  #depth.max() around 550-600 lol
            scaler = 0.0208
        else:
            scaler = 1 #Leave it be then
        print_flush ("Torch model loading into device name: ", torch.cuda.get_device_name(DEVICE))
        model = load_model(args_god.encoder, args_god.encoder_path, DEVICE)
        ...
        while True:
            ...
            torch.cuda.empty_cache()
    #profiler = LineProfiler()#profiler.add_function(inference_worker_nested)#profiler.enable() #dump_line_profile_to_csv(profiler, filename=DebugDir + f"inference_worker_{os.getpid()}_Profiler.csv")
...
def nibba_woka(begin, end, job_queue, result_queue, gpu_notify_list, args_god, ffmpeg_config = None):
    #Silence all output of child process
    redirrect_stdout(args_god.DebugDir + str (begin//(end-begin))+'_' + str(begin)+'.txt')
    SubClipDir = args_god.SubClipDir
    ...
    try:
        ...
        redirrect_stdout(args_god.DebugDir + 'ERROR.txt')
...
def we_ballin (args_god):
    ...
    inference_workers = [Process(target=inference_worker, args=(inference_job_queue_list[i], inference_result_queue_list[i], notify_queue_list[i],
                                                                torch.device('cuda', (i%num_gpu)), args_god
                                                                )
                                 )
    ...
    for idx, i in enumerate(frame_indices):
        ...
        worker = Process(target = nibba_woka, args = (i, min(end_frame, i + step),
                                                      job_queue_list[idx], result_queue_list[idx], [notify_queue_list[gpu_worker_number], within_gpu_worker_inference_idx],
                                                      args_god, ffmpeg_config
                                                      ))
